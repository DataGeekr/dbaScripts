...
SELECT 'Test-DbaLastBackup -SqlInstance DRACO -Destination CANIS -IgnoreLogBackup -AzureCredential ''OabAzureBlobStorage'' -Database ' + STUFF((
            ...
            FOR XML PATH (''), TYPE).value('text()[1]','nvarchar(max)'),1,1,N'') + ' | Export-Csv "D:\DRACO.RestorePlan.Cloud.' + CONVERT(CHAR(8), GETDATE(), 112) + '.csv"'
UNION ALL
SELECT 'Test-DbaLastBackup -SqlInstance DRACO -Destination CANIS -IgnoreLogBackup -CopyFile -CopyPath "\\CANIS\Restore" -Database ' + STUFF((
            SELECT DISTINCT CONCAT(N',', d.name )
            FROM Master.sys.databases d
                 LEFT JOIN (
                           SELECT DISTINCT
            ...
            FOR XML PATH (''), TYPE).value('text()[1]','nvarchar(max)'),1,1,N'') + ' | Export-Csv "D:\DRACO.RestorePlan.Local.' + CONVERT(CHAR(8), GETDATE(), 112) + '.csv"'